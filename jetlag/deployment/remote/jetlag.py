...
class Universal:
    ...
    def load(self, backend, notify=None, jetlag_id=None):
        ...
        if jetlag_id is None or jetlag_id.strip().lower() in [
                "none", "unknown", ""
        ]:
            return
        ...
        self.values['storage_id'] = "%s-%s-storage-%s" % (
            g.group(1), g.group(2), g.group(3))
        self.values['execm_id'] = "%s-%s-exec-%s" % (g.group(1), g.group(2),
                                                     g.group(3))
        self.values['forkm_id'] = "%s-%s-fork-%s" % (g.group(1), g.group(2),
                                                     g.group(3))
        self.values['app_id'] = "%s-%s_queue_%s-1.0.0" % (
            g.group(1), g.group(2), g.group(3))
        self.values['fork_app_id'] = "%s-%s_fork_%s-1.0.0" % (
            g.group(1), g.group(2), g.group(3))
    ...
    def check_values(self, values):
        ...
        for k in values:
            ...
            assert values[k] == self.values[k], 'k: ' + k + ' = ' + str(
                values[k]) + " != " + str(self.values[k])
    ...
    def initf(self,utype,machine,machine_user,domain,port:int=22,
        ...
        self.values["app_name"] = self.fill(
            "{machine}-{machine_user}_queue_{other}")
        self.values["fork_app_name"] = self.fill(
            "{machine}-{machine_user}_fork_{other}")
        self.values["storage_id"] = self.fill(
            '{machine}-{machine_user}-storage-{other}')
        self.values["execm_id"] = self.fill(
            '{machine}-{machine_user}-exec-{other}')
        self.values["forkm_id"] = self.fill(
            '{machine}-{machine_user}-fork-{other}')
        self.mk_extra()
    ...
    def init(self, **kwargs):
        ...
        for k in self.values:
            assert self.values[
                k] != "unknown", "Please supply a string value for '%s'" % k
            assert self.values[
                k] != -666, "Please supply an integer value for '%s'" % k
        self.values["storage_id"] = self.fill(
            '{machine}-{machine_user}-storage-{other}')
        self.values["execm_id"] = self.fill(
            '{machine}-{machine_user}-exec-{other}')
        ...
        name = "machine-config-" + self.values["other"] + "-" + self.values[
            "jetlag_id"]
        mm = {"name": name, "value": machine_meta}
    ...
    def set_backend(self):
        ...
        for bk in ["user", "pass", "tenant", "utype"]:
            assert bk in backend.keys(
            ), "key '%s' is required in backend: %s" % (bk, str(backend))
    ...
    def get_auth_file(self):
        ...
        burl = codeme("~".join([
            ...
            self.values["backend"]["baseurl"], self.values["backend"]["utype"],
            self.values["backend"]["user"]
        ]))
        if self.values['utype'] == 'tapis':
            auth_file = os.environ[
                "HOME"] + "/.tapis/" + user + "/" + burl + "/current"
        else:
            auth_file = os.environ[
                "HOME"] + "/.agave/" + user + "/" + burl + "/current"
        return auth_file
    ...
    def get_storage(self):
        ...
        response = requests.get(self.fill('{apiurl}/systems/v2/{storage_id}'),
                                headers=headers)
    ...
    def mk_storage(self, force=False):
        ...
        if storage_update or (not self.check_machine(storage_id)):
            ...
            response = requests.post(self.fill('{apiurl}/systems/v2/'),
                                     headers=headers,
                                     data=json_storage)
            check(response)
    ...
    def files_list(self, dir):
        ...
        pause()
        response = requests.get(
            self.fill('{apiurl}/files/v2/listings/system/{storage_id}//' +
                      dir),
            headers=headers,
            params=params)
        check(response)
    ...
    def job_by_name(self, name):
        headers = self.getheaders()
        params = (('name', name), )
        response = requests.get(self.fill('{apiurl}/jobs/v2'),
                                headers=headers,
                                params=params)
        check(response)
        return response.json()["result"]
    ...
    def job_cleanup(self):
        ...
        for f in fdata:
            if f["format"] == "folder":
                ...
                response = requests.delete(self.fill(
                    '{apiurl}/files/v2/media/system/{storage_id}/{job_dir}/' +
                    job_name),
                                           headers=headers)
                check(response)
    ...
    def create_token(self):
        ...
        auth = (self.values["sys_user"], self.values["sys_pw"])
        ...
        response = requests.post(self.fill('{apiurl}/token'),
                                 data=data,
                                 auth=(c_key, c_secret))
        jdata = response.json()
    ...
    def refresh_token(self):
        ...
        auth = (auth_data['apikey'], auth_data['apisecret'])
        try:
            response = requests.post(self.fill('{apiurl}/token'),
                                     headers=headers,
                                     data=data,
            ...
            auth_data["expires_at"] = datetime.utcfromtimestamp(ts).strftime(
                '%c')
    ...
    def mk_execution(self, force=False):
        ...
        execm = {
            ...
            execm_id,
            "name":
            "{machine} exec ({machine_user})",
            "description":
            "The {machine} execution computer",
            "site":
            "{domain}",
            "public":
            False,
    ...
    def make_dir(self, dir_name):
        ...
        response = requests.put(
            self.fill('{apiurl}/files/v2/media/system/{storage_id}/'),
            headers=headers,
            data=data)
        check(response)
    ...
    def file_upload(self, dir_name, file_name, file_contents=None):
        ...
        headers = self.getheaders()
        files = self.fill({'fileToUpload': (file_name, file_contents)})
        if self.values["utype"] == 'tapis':
            url = self.fill(
                '{apiurl}/files/v2/media/system/{storage_id}//{home_dir}/' +
                dir_name)
        else:
            url = self.fill('{apiurl}/files/v2/media/system/{storage_id}/' +
                            dir_name)
        pause()
        pause()
    ...
    def mk_app(self, force=True):
        ...
        app = {
            ...
            "parameters": [{
                ...
                "details": {
                    ...
                    "description":
                    "The Singularity image to run: swan, funwave",
                    "argument": None,
                    "showArgument": False,
        ...
        data = json.dumps(app)
        headers = self.getheaders(data)
        response = requests.post(self.fill('{apiurl}/apps/v2/'),
                                 headers=headers,
                                 data=data)
        check(response)
    ...
    def hello_world_job(self, jtype='fork', sets_props={}, needs_props={}):
        ...
        return self.run_job('hello-world',
                            input_tgz,
                            jtype=jtype,
                            run_time="00:01:00",
                            sets_props=sets_props,
    ...
    def run_job(self,
        ...
        if ready:
            ...
            headers = self.getheaders(data)
            pause()
            response = requests.post(self.fill('{apiurl}/jobs/v2/'),
                                     headers=headers,
                                     data=data)
            check(response)
            rdata = response.json()
            ...
            meta = {"name": "jobdata-" + job_name, "value": data}
            self.set_meta(meta)
            return jobid
        else:
            data = {
        ...
        for data in self.get_meta('jobdata-.*'):
            ...
            if "jobid" in m:
                ...
                if m["status"] == "FINISHED":
                    ...
                    try:
                        f = self.get_file(m["jobid"],
                                          "run_dir/return_code.txt")
                    except:
                        f = b'EXIT(616)'
                ...
                if done:
                    ...
                    try:
                        response = requests.delete(
                            self.fill('{apiurl}/files/v2/media/system/') +
                            jloc,
                            headers=headers)
                        if response.status_code in success_codes:
                            ...
                            print("file gone (status_code=%d)" %
                                  response.status_code)
                            self.del_meta(data)
                        else:
                            print("failed (status_code=%d)" %
                                  response.status_code)
    ...
    def del_meta(self, data):
        headers = self.getheaders()
        uuid = data['uuid']
        response = requests.delete(self.fill('{apiurl}/meta/v2/data/') + uuid,
                                   headers=headers)
        check(response)
    ...
    def set_meta(self, data):
        ...
        response = requests.post(self.fill('{apiurl}/meta/v2/data/'),
                                 headers=headers,
                                 files=files)
        check(response)
        jdata = response.json()
    ...
    def get_meta(self, name):
        ...
        response = requests.get(self.fill('{apiurl}/meta/v2/data'),
                                headers=headers,
                                params=params)
        check(response)
    ...
    def job_status(self, jobid):
        ...
        # pause()
        response = requests.get(self.fill("{apiurl}/jobs/v2/") + jobid,
                                headers=headers)
        if response.status_code == 404:
            return None
        check(response)
    ...
    def job_stop(self, jobid):
        ...
        headers = self.getheaders(data)
        pause()
        response = requests.post(self.fill("{apiurl}/jobs/v2/") + jobid,
                                 headers=headers,
                                 data=data)
        check(response)
    ...
    def get_file(self, jobid, fname, as_file=None):
        ...
        for header in response.headers:
            if header.lower() == "content-type":
                if response.headers[header].lower(
                ) == "application/octet-stream":
                    is_binary = True
        if as_file is not None:
            if is_binary:
                fd = os.open(as_file, os.O_WRONLY | os.O_CREAT | os.O_TRUNC,
                             0o0644)
                os.write(fd, content)
                os.close(fd)
            else:
    ...
    def install_key(self):
        ...
        """
        headers = self.getheaders()
        response = requests.get(self.fill(
            '{apiurl}/files/v2/media/system/{storage_id}/.ssh/authorized_keys'
        ),
                                headers=headers)
        ...
        if self.public_key not in auth_file:
            ...
            self.file_upload('.ssh',
                             'authorized_keys',
                             file_contents=new_auth_file)
            print("DONE")
        else:
    ...
    def get_app_pems(self, app='queue'):
        ...
        url = self.fill("{apiurl}/apps/v2/" + app_name + "-" + version +
                        "/pems")
        response = requests.get(url, headers=headers)
        if response.status_code == 403:
    ...
    def access(self, user, allow):
        ...
        meta_name = self.fill("machine-config-" + self.values["sys_user"] +
                              "-" + self.values["jetlag_id"])
    ...
    def systems_list(self):
        headers = self.getheaders()
        response = requests.get(self.fill("{apiurl}/systems/v2/"),
                                headers=headers)
        check(response)
        jdata = response.json()["result"]
        return jdata
    ...
    def apps_list(self):
        headers = self.getheaders()
        response = requests.get(self.fill("{apiurl}/apps/v2/"),
                                headers=headers)
        check(response)
        jdata = response.json()["result"]
        return jdata
    ...
    def jetlag_ids(self):
        ...
        for a in self.apps_list():
            ...
            g = re.match(r'^(\w+)-(\w+)_(fork|queue)_(\w+)-(\d+\.\d+\.\d+)',
                         a['id'])
            if g:
                key = "%s-%s-%s" % (g.group(1), g.group(2), g.group(4))
                if g.group(3) == 'fork':
